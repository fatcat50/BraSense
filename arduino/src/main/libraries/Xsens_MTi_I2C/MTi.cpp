...
void MTi::configureOutputs() {
  ...
  if (xbus.productCode == '1') {//MTi-1 IMU
    ...
    //uint8_t outputConfig[] = {0xC0, 0x04, 0x20, 0x30, 0x00, 0x32};                          //setOutputConfiguration Xbus message with Data field 0x20 0x30 0x00 0x01 (EulerAngles, 32-bit float, at 10 Hz)
    //Serial.println("Configuring Euler angles at 50 Hz.");
    Serial.println("Configuring Euler angles at 100 Hz.");
    uint8_t outputConfig[] = {0xC0, 0x04, 0x20, 0x30, 0x00, 0x64};                          //setOutputConfiguration Xbus message with Data field 0x20 0x30 0x00 0x01 (EulerAngles, 32-bit float, at 100 Hz)
